...
public class HangSubsystem extends SubsystemBase implements TestableSubsystem {
  ...
  public HangSubsystem(int hangMotorId) {
    ...
    hangMotorConfigs = Utils.configureTalonGains(m_hangMotor, 0, 0, 0, 0, 0);
   ...
   * 
   * <p>
   * {@code kIsGrounded} represents the hang being at the lowest level, nearly on
   * the ground
   * <p>
   * {@code kIsGrounding} represents the hang moving towards the above state
   * <p>
   * {@code kIsL1} represents the hang moving towards the L1 position, readying
   * for hanging
   * <p>
   * {@code kIsGoingToL1} represents the hang mechanism moving towards the
   * previous position
   * <p>
   * {@code kCustom} represents guh
   */
  ...
  public TestCommandBase getTestCommand() {
    ...
    return new TestCommandBase() {
      ...
      public void initialize() {
        ...
        maxAllowedError = HangConstants.kTestTargetHangle.times(HangConstants.kMaxTestHangErrorPercentage / 100); // ty for using wpilib units 🥺
      ...
      public TestResult getCurrentResult() {
        ...
        Angle currentHangAngle = m_hangMotor.getPosition().getValue();
        if (Timer.getFPGATimestamp() - startTime >= HangConstants.kMaxTesthangTimeToReachHeight) {
          output = "The hang took too long to reach desired height.";
          return TestResult.KNOWN_FAILURE;
        }
        if (currentHangAngle.isNear(HangConstants.kTestTargetHangle, maxAllowedError) ) {
          output = "This hang motor moved to the specified loacation within the time constraint and without to muh deviation.";
